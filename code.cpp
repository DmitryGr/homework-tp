...
class ArmyFactory{
    public:
        virtual void create_infantry(vector<Infantry*> &arr) = 0;
        virtual void create_tanks(vector<Tank*> &arr) = 0;
        virtual void create_artillery(vector<Artillery*> &arr) = 0;
...
class SovietArmyFactory : public ArmyFactory{
    public:
        void create_infantry(vector<Infantry*> &arr){
            for (int i=0; i < SOVIET_SOLDIERS; ++i){
                arr.push_back(new Infantry());
            }
        }
        void create_tanks(vector<Tank*> &arr){
            for (int i=0; i < SOVIET_TANKS; ++i){
                arr.push_back(new Tank());
            }
        }
        void create_artillery(vector<Artillery*> &arr){
...
class Game{
    public:
        Army* create_army(ArmyFactory &factory){
            ...
            vector<Infantry*> inf;
            factory.create_infantry(inf);
            vector<Tank*> tanks;
            factory.create_tanks(tanks);
            vector<Artillery*> art;
            factory.create_artillery(art);
            new_army->add_soldiers(inf);
            new_army->add_tank(tanks);
            new_army->add_artillery(art);
            return new_army;
        }
};
